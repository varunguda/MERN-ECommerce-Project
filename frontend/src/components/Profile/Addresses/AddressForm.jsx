...
const AddressForm = ({ onSubmit, address, setAddress, disableSaveBtn, validateFields, cancelClickHandler, module }) => {
    ...
    const setStateCodeHandler = (e) => {
        module.State && module.State.getStatesOfCountry("IN").forEach((state) => {
    ...
    return (
        ...
        module && (<form onSubmit={onSubmit} method="post">
            ...
            <div className='input-section'>
                ...
                >
                    <option value=""></option>
                    {module.City && module.City.getCitiesOfState("IN", address.state_code).map((city, index) => {
                        return <option key={index}>{city.name}</option>
                    })}
                </select>
                {(validateFields && !!cityValidator(address.city, address.state_code, module.City)) && (
                    <span className='input-error'>{cityValidator(address.city, address.state_code, module.City)}</span>
                )}
            </div>
            ...
            <div className='input-section'>
                ...
                >
                    <option value=""></option>
                    {module.State && module.State.getStatesOfCountry("IN").map((state, index) => {
                        return <option key={index}>{state.name}</option>
                    })}
                </select>
                {(validateFields && !!stateValidator(address.state, module.State)) && (
                    <span className='input-error'>{stateValidator(address.state, module.State)}</span>
                )}
            </div>
